...
from app.services.extreme_scanner import ExtremeAlert, ExtremeScanResult
...
class ExtremePaperTradingService:
    ...
    def process_scan(
        ...
        for symbol in symbols:
            candles = self.bridge.candles(account, symbol, scan.timeframe, 80)
            if not candles:
                continue
            candles_by_symbol[symbol] = candles
            prices[symbol] = candles[-1]

        opportunities: list[MarketOpportunity] = []
        for rank, alert in enumerate(alerts, start=1):
            reading = readings.get(alert.symbol)
            candles = candles_by_symbol.get(alert.symbol)
            if reading is None or not candles:
                continue
            opportunity = self._to_opportunity(rank, alert, reading.price, candles)
    ...
    def _to_opportunity(
        ...
        reasons = [
            ...
            SignalReason(
                "rsi",
                f"RSI(1) reached {alert.rsi1:.2f}; this is a highly reactive extreme condition.",
                impact,
                0.2,
            ),
            SignalReason(
                "macd",
                f"MACD(5,6,1) confirms {action}-side momentum at {alert.macd:.8g}.",
                impact,
                0.25,
            ),
            SignalReason(
                "trend",
                "Fast moving average confirms the reversal direction against the extreme.",
                impact,
                0.2,
            ),
        ]
        return MarketOpportunity(
            rank=rank,
            symbol=alert.symbol,
            description=f"{alert.symbol} confirmed extreme reversal",
            category="Extreme scanner",
            direction=direction,
            confidence=0.88,
            entry=entry,
