            ...
            <div className="simulation-assumptions">
              <div className="paper-subheading">
                <div>
                  ...
                  <span>{extreme ? "Applied only after RSI(1), MACD, and MA confirmation" : jdub ? "15m New York range, 5m close, then 1m trigger" : rigorgate ? "BUY opens a long; WAIT does nothing; SELL closes a matching long" : candlestick ? "M15 engulfing opens BUY or SELL; ATR sets SL and 1.35R sets TP" : "Applied to every eligible directional signal on the selected timeframe"}</span>
                </div>
              </div>
              <dl>
                <div><dt>Market coverage</dt><dd>{engine.scanned_symbols || "--"} instruments</dd></div>
                <div><dt>Entry filter</dt><dd>{extreme ? `Confirmed 85/15 · score ${engine.minimum_opportunity_score}+` : jdub ? `Opening range · score ${engine.minimum_opportunity_score}+` : rigorgate ? `BUY / SELL gate · score ${engine.minimum_opportunity_score}+` : candlestick ? `M15 engulfing -> BUY / SELL · score ${engine.minimum_opportunity_score}+` : engine.minimum_opportunity_score === 0 ? "All strategy signals" : `Score ${engine.minimum_opportunity_score}+`}</dd></div>
